...
TEST(VectorTest, Begin_ValidVector) {
    s21::vector<int> v = {1, 2, 3, 4, 5};
    // auto позволяет не указывать тип переменной явно, говоря компилятору, чтобы он сам определил
    // фактический тип переменной, на основе типа инициализируемого значения.
    auto it = v.begin();
    EXPECT_EQ(*it, 1);
...
TEST(VectorTest, MaxSize) {
    const size_t size = 0;
    const s21::vector<int> v;
    // You can't predict the exact max size, but it should be a large value
    EXPECT_GT(v.max_size(), size);
}
